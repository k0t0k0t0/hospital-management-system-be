...
export class WardService {
  async assignPatientToBed(
    ...
    try {
      ...
      // Update bed status
      await wardRepository.updateBedStatusAsync(bedId, "occupied", patientId);
  ...
  async dischargePatient(assignmentId: string) {
    try {
  ...
  async getWardStatus(wardId: string) {
    try {
      const ward = await wardRepository.findWardByIdAsync(wardId);
      if (!ward) {
        return ServiceResponse.failure(
  ...
  async getAllWards() {
    try {
      const wards = await wardRepository.findAllWardsAsync();
      return ServiceResponse.success("All wards retrieved successfully", wards);
    } catch (error) {
      return ServiceResponse.failure(
