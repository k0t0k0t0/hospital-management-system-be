...
export class PatientRepository {
  ...
  async findAllAppointmentsAsync(): Promise<Appointment[]> {
    ...
    // Get all doctors and patients in parallel
    const [doctors, patients] = await Promise.all([
      Models.Doctor.find({ _id: { $in: doctorIds } })
    ...
    return appointments.map((apt) => ({
      ...
      doctor: {
        id: apt.doctorId.toString(),
        firstName: doctorMap.get(apt.doctorId.toString())?.firstName || "",
        lastName: doctorMap.get(apt.doctorId.toString())?.lastName || "",
      },
      patient: {
        id: apt.patientId.toString(),
