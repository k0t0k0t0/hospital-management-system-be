...
export class PatientService {
  ...
  async createAppointment(
    ...
    try {
      ...
      console.log("Looking up doctor:", appointmentData.doctorId);

      // Verify doctor availability
      const doctor = await staffRepository.findByIdAsync(
        appointmentData.doctorId
      );
      console.log("Found doctor:", doctor); // See what we're getting back

      // Check if doctor exists and has correct role
      // if (!doctor || doctor.role !== "doctor") {
      //   return ServiceResponse.failure(
      //     `Doctor not found or invalid role. Found: ${doctor?.role}`,
      //     null,
      //     StatusCodes.NOT_FOUND
      //   );
      // }
      ...
      logError("PatientService.createAppointment", error);
      return ServiceResponse.failure(
  ...
  async getAllAppointments() {
    try {
      const appointments = await patientRepository.findAllAppointmentsAsync();
      return ServiceResponse.success(
